...
A live, editable catalog for physical media (VHS · DVD · CD · cassette · and more).

- **Public visitors** see searchable, filterable, sortable collection and wish
  list views with thumbnails and a detail page for every item.
- **You** (the owner) log in with a one-time email link and can **add, edit, and
  delete** listings, move items between the collection and wish list, and upload
  thumbnail images — right from the website.
...
| Supabase database | Stores every listing in the `listings` table, separated by `list`. |
...
2. Browse / search the collection or wish list. Tap any item for its own page.
3. To make changes, tap **Owner login**, enter your email, and click the link
   that lands in your inbox.
4. Once signed in you'll see a **Manage** button — add new listings there, choose
   whether they belong to the collection or wish list, or open any item and tap
   **Edit** / **Delete**. Use **Upload image** to attach a thumbnail.

## Existing database migration

If your Supabase database was created before the wish list existed, run
`db/migrations/0001_add_list_column.sql` once in the Supabase SQL Editor. It adds
the `list` column with a default of `collection`, so existing saved rows stay in
your collection.
